...
describe('Collide 1.0 workbench shell', () => {
  it('renders the systems-lab workbench as the first screen', () => {
    ...
    expect(screen.getByRole('button', { name: /Bank Conflicts/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Divergence/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Reduce \/ Scan/ })).toBeInTheDocument()
  ...
  it('switches labs and shows lab-specific metrics', async () => {
    ...
    render(<App />)
    await user.click(screen.getByRole('button', { name: /Bank Conflicts/ }))
  ...
  it('keeps the central canvas meaningful for occupancy', async () => {
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByRole('button', { name: /Occupancy/ }))
